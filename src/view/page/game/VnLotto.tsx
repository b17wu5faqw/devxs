...
const VnLotto = () => {
  ...
  const { listDraw } = useListDraw(Number(id), 1);
  ...
  }, [selectDraw, selectedButton, typeId]);

  // const { lastDraw } = useLastDraw(selectDraw ? Number(selectDraw) : 0, typeId ?? 0);
  useEffect(() => {
    fetchLastDraw();
  ...
  const fetch = useCallback(async () => {
    const resp = await getTypeV2({ regionId: region, schedulerId: Number(id), type: 1 });
    if (resp.status == 1) {
      setType(resp.data);
      if (activeTab === "4D") {
      ...
      >
        <Flex>
          ...
          <Flex>
            <CustomText
              onClick={() =>
                typeId == 1 ? openModal(MODAL.GAME) : openModal(MODAL.GAME_LO)
              }
              sx={{
