...
const AnyTimePokerContent: React.FC<AnyTimePokerProps> = ({ gType = 166 }) => {
  ...
  const { gameState } = useLiveKu(gType);
  const gameIdNumber = gType ? gType : 166;
    ...
    >
      ...
      <Box sx={{ width: "200px", backgroundColor: baseColors.white }}>
        ...
        <div className="bg-[#b3d2f2] border-t-[1px] border-t-[#fff] h-[110px] p-[8px_10px]">
          <div>
            <span>Tỉ lệ :</span>
            <span className="color_red">{odds ? odds : subType?.prize_rate}</span>
          </div>
          <div>
      ...
      <PopupConfirm
        ...
        prizeRate={odds ? odds : (subType?.prize_rate ? Number(subType.prize_rate) : 0)}
...
export default AnyTimePokerContent;
