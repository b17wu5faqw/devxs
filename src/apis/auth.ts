...
export const getAccountBalance = async (jwt_key: string): Promise<any> => {
  const resp = await post(getUrl(`account/get-balance`), {
    jwt_key: jwt_key,
  });
  return resp.data;
};
...
export const getAccountInfo = async () => {
  const resp = await get(getUrl(`Account/GetAccountInfo`));
  return resp;
};
