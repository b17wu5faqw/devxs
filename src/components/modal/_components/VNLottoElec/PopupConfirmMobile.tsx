...
const PopupConfirmMobile: React.FC<PopupConfirmProps> = ({
  ...
  const isOpenGlobal = useModalStore((state) =>
    state.isModalOpen(MODAL.CONFIRM)
  );
  ...
  useEffect(() => {
    setTotalChip(betChip * (rate ? Number(rate) : 1) * numbers.length);
  }, [betChip, numbers, rate]);

  useEffect(() => {
    setTotalPrize(betChip * (prizeRate ? Number(prizeRate) : 0));
  }, [betChip, prizeRate]);
  ...
  const formatNumbersForDisplay = (numbers: string[]): string => {
    return numbers.join(", ");
  };
      ...
      >
        ...
        <Box>
          ...
          <Flex>
            ...
            >
              Số kỳ: <span className="text-[#3287e4]">1</span>
            </CustomText>
          </Flex>
          <Flex>
            <CustomText
