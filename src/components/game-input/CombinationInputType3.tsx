...
export const CombinationInputType3 = ({
  ...
  const handleNumberClick = (number: string) => {
    if (!selectedNumbers.includes(number)) {
      ...
      const requiredNumbers = selectedTab + 4;
  ...
  const getTabValue = (tabGroup: "first" | "second") => {
    if (tabGroup === "first") {
      return selectedTab < 3 ? selectedTab : -1;
    }
    return selectedTab >= 3 ? selectedTab - 3 : -1;
  };
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Box sx={{ padding: " 5px 10px" }}>
        ...
        <Tabs
          value={getTabValue("second")}
          onChange={(_, newValue) => handleTabChange(_, newValue + 3)}
          sx={{
            minHeight: "auto",
            "& .MuiTabs-flexContainer": {
              gap: "5px",
